...
# To get a specific valance electron of an element
elif question == 2:
    #wrkbk2 = openpyxl.load_workbook("Valance.xlsx")
    #sh2 = wrkbk2.active
...
3 : Different type of elements in periodic table """)
    ...
    if which_topic_no == 1: # 1 A electron Configuration
        print('There are many explanations for the term "Electron Configuration". But the best way to describe it is by how wikipedia talks about it." Electronic configurations describe each electron as moving independently in an orbital, in an average field created by all other orbitals."')
# part A video
...
# part AB       
        if askec == "y" or "yes": # To watch a video explaining electron configuration
            ...
            askec2 =str(input("Would you like to find the elctron configuaration of an element of you choice"))
            askec2l = askec2.lower()
# part  AC            
            if askec2l == "y" or "yes": # to check
                #wrkbk3 = openpyxl.load_workbook("electron Configuration.xlsx")  
                #sh3 = wrkbk3.active
...
# part  B else
            ...
            if askec4l == "y" or "yes": # to check
                #wrkbk4 = openpyxl.load_workbook("electron Configuration.xlsx")
                #sh4 = wrkbk4.active
                n = int(input("""what element:
Hydrogen	1	, Helium  	2	, Lithium  	3	, Beryllium	4	, Boron   	5	, Carbon  	6	, Nitrogen	7	, Oxygen  	8	,
Fluorine	9	, Neon    	10	, Sodium   	11	, Magnesium	12	, Aluminum	13	, Silicon  	14	, Phosphorus	15	, Sulfur  	16	,
...
The number of the element you want to know about"""))
        ...
        a = str(input("Do you want to seethe Periodic table"))
        al = a.lower()
        if al == "yes" or "y":
            webbrowser.open_new("https://www.google.com/search?q=periodic+table&rlz=1C1CHBF_enUS980US980&oq=pe&aqs=chrome.2.69i59l3j69i57j69i60l4.3554j0j7&sourceid=chrome&ie=UTF-8")
        else:
            print("Start again")

    elif which_topic_no == 3: # 3 A
        print("""On the periodic table, there are families which are groups of elements with similar properties. These families are alkali metals, alkaline earth metals, transition metals, post-transition metals, metalloids, halogens, noble metals, and noble gases""")
        q = str(input("Do you want to seethe Periodic table"))
        qt = q.lower()
        if qt == "yes" or "y":
            print("""
...
The noble gases, also called aerogens, are inert gases. Some examples include argon, krypton, and neon. They can be found in group eighteen on the periodic table. Likewise, this means they have a complete valence shell. For this reason, they are stable and relatively unreactive. Furthermore, the noble gases have low boiling points and low melting points. Physically they are colorless and have no smell.""")
    ...
    #elif which_topic_no == 2: # 2 A
        #pass    
#for row in range(2, 3):
   # for col in range(1,5):0
      #  char = get_column_letter(col)
      #  print(work_sheet[char + str(row)].value)
